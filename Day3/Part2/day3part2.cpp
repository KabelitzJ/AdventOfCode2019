...
int min(std::vector<Point> vec, std::map<Point, int> firstPath, std::map<Point, int> secondPath) {
    int length = INT_MAX;

    for(int i = 0; i < vec.size(); i++) {
        int temp = firstPath[vec[i]] + secondPath[vec[i]];
        if(temp <= length) {
            length = temp; 
        }
    }
...
int main(void) {
    ...
    int a = min(intersections, firstPath, secondPath);
